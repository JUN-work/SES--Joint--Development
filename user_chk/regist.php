...
{
    if ($GLOBALS['name'] == '') :
        $GLOBALS['error_name'] = "ユーザーネームが入力されていません。";
    elseif (mb_strlen($GLOBALS['name']) > 20) :
        $GLOBALS['error_name'] = "ユーザーネームは20文字以内で入力して下さい。";
    endif;
    return true;
}
...
{
    if ($GLOBALS['password'] == '') :
        $GLOBALS['error_password'] = "パスワードが入力されていません。";
        return false;
    elseif (!preg_match('/^[0-9a-zA-Z]{5,20}$/', $GLOBALS['password'])) :
        $GLOBALS['error_password'] = "パスワードは半角英数字を使用し、5文字以上20文字以下で入力して下さい。";
        return false;
    endif;
    return true;
}
